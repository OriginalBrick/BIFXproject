...
age
```
Mass starts as 47.5 and goes to 109.25.
Age starts at 122 and goes to 102.
...
![](img/BIFXproject - Intro to R - Project Creation.png)
...
```{r}
# ?c tells us that vector type is determined by the data used.
# typeof() lets us easily figure out the type of each vector.
c(1, 2, 3)
typeof(c(1, 2, 3))
c('d', 'e', 'f')
typeof(c('d', 'e', 'f'))
...
Using ?paste, we can see that `sep` is used to separate the terms, while `collapse` is used to separate the results.
...
One could use either the read.table or read.delim. These let you specify both `dec` for decimal type and `sep` for delimiter type.
