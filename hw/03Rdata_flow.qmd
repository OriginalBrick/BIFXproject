...
D. `m[2,c(4,5)]` - This is the only correct colution.
...
1.  Why does `gapminder[1:20]` return an error? How does it differ from `gapminder[1:20, ]`?
    ...
    The first command doesn't work because there aren't 20 columnsg. The second command works because the range of 20 is applying only to rows, of which there are far more than 20.
...
{
  print("Gapminder records EXIST for 2002.")
}else{
  print("Gapminder records DO NOT EXIST for 2002.")
}
```
...
{
  print("Gapminder records EXIST for 2012.")
}else{
  print("Gapminder records DO NOT EXIST for 2012.")
}
```
...
```{r}
output_vector <- matrix(NA,nrow=5,ncol=8)
for(i in 1:5)
{
  for(j in 1:8)
  {
    output_vector[i,j] <- paste0(i,letters[j])
  }
}
output_vector
```
...
```{r}
#Easiest way to do this is using t().
output_vector
